GV_OD_query="""select date_format(date_parse(cast("order_date" as varchar) , '%Y-%m-%d'), '%Y-%m') as month, 'Voucher' as domain_name, 'OD_with_base_query' as dashboard, 'shared_open_data_gift_voucher_order' as view_name,
...
FROM "default".shared_open_data_gift_voucher_order odv
WHERE odv."seller np name" NOT IN ('gl-6912-httpapi.glstaging.in/gl/ondc')
and (DATE(odv."o_created_at_date")) >= DATE('2023-11-01')
GROUP BY odv."network order id") group by date_format(date_parse(cast("order_date" as varchar) , '%Y-%m-%d'), '%Y-%m') """

B2B_OD_query="""select date_format(date_parse(cast("order_date" as varchar) , '%Y-%m-%d'), '%Y-%m') as month, 'B2B' as domain_name, 'OD_with_base_query' as dashboard, 'shared_open_data_b2b_order' as view_name,
count(1) as row_count,
count(distinct "network_order_id") as distinct_order_count from(
SELECT odv."network order id" AS network_order_id,
...
WHERE odv."seller np name" NOT IN ('gl-6912-httpapi.glstaging.in/gl/ondc')
and date(date_parse("O_Created Date & Time",'%Y-%m-%dT%H:%i:%s')) >= DATE('2023-11-01')
GROUP BY odv."network order id") group by date_format(date_parse(cast("order_date" as varchar) , '%Y-%m-%d'), '%Y-%m')"""

B2C_OD_query="""select date_format(date_parse(cast("order_date" as varchar) , '%Y-%m-%d'), '%Y-%m') as month, 'B2C' as domain_name, 'OD_with_base_query' as dashboard, 'nhm_order_fulfillment_subset_v1' as view_name,
count(1) as row_count,
count(distinct "network_order_id") as distinct_order_count from(
WITH order_subcategories AS (
    ...
    from
      ...
      where date(date_parse("O_Created Date & Time", '%Y-%m-%dT%H:%i:%s')) >= DATE('2023-11-01')
...
FROM "default".nhm_order_fulfillment_subset_v1  odv
...
logistics_OD_query="""select date_format(date_parse(cast("order_date" as varchar) , '%Y-%m-%d'), '%Y-%m') as month, 'Logistics' as domain_name, 'OD_with_base_query' as dashboard, 'shared_open_data_logistics_order' as view_name,
count(1) as row_count,
...
WITH base_data AS (
    ...
    and DATE(a.order_created_at) >= date('2023-11-01')
...
GV_NO_query="""SELECT date_format(date_parse(CAST(o_created_at_date AS VARCHAR), '%Y-%m-%d %H:%i:%s.%f'), '%Y-%m') as month,'Voucher' as domain_name, 'NO_with_base_query' as dashboard, 'shared_gift_card_order_fulfillment' as view_name,
    count(1) as row_count,
    COUNT(DISTINCT network_order_id) as distinct_order_count
FROM 
    "default".shared_gift_card_order_fulfillment
GROUP BY 
    date_format(date_parse(CAST(o_created_at_date AS VARCHAR), '%Y-%m-%d %H:%i:%s.%f'), '%Y-%m')"""
B2B_NO_query="""select date_format(date_parse(cast("Date" as varchar) , '%Y-%m-%d'), '%Y-%m') as month, 'B2B' as domain_name, 'NO_with_base_query' as dashboard, 'shared_order_fulfillment_nhm_fields_view_hudi' as view_name,
count(1) as row_count,
...
B2C_NO_query="""select date_format(date_parse(cast("Date" as varchar) , '%Y-%m-%d'), '%Y-%m') as month, 'B2C' as domain_name, 'NO_with_base_query' as dashboard, 'shared_order_fulfillment_nhm_fields_view_hudi' as view_name,
count(1) as row_count,
count(distinct "Network order id") as distinct_order_count from (
...
logistics_NO_query="""select date_format(date_parse(cast("Date" as varchar) , '%Y-%m-%d'), '%Y-%m') as month, 'Logistics' as domain_name, 'NO_with_base_query' as dashboard, 'shared_logistics_item_fulfillment_view_with_date' as view_name,
count(1) as row_count,
...
AND (CASE 
	...
	ELSE shipment_type END) = 'P2P') a WHERE (a.bap_id NOT IN ('ondcpreprodtest.sellerapp.in', 'integrations-preprod.channelier.com', 'ondcpreprod.sellerapp.in', 'preprod-ondc.viranc.com/p/v1/store', 'ondc.testtoprod.production.com', 'biz.test.bitsila.com') or a.bap_id is null)
...
B2C_L1_query="""select to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date as month, 'B2C' as domain_name, 'OD_with_L1_query' as dashboard, 'fact_order_detail' as view_name,
COUNT(distinct "network_order_id") as distinct_order_count,
count(1) as row_count
from dev.fact_order_detail fod 
group by to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date"""
B2B_L1_query="""select to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date as month, 'B2B' as domain_name, 'OD_with_L1_query' as dashboard, 'fact_order_detail_b2b' as view_name,
COUNT(distinct "network_order_id") as distinct_order_count,
count(1) as row_count
from dev.fact_order_detail_b2b fodbb  
group by to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date"""
GV_L1_query="""select to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date as month, 'Voucher' as domain_name, 'OD_with_L1_query' as dashboard, 'retail_voucher_fact_order_detail' as view_name,
COUNT(distinct "network_order_id") as distinct_order_count,
count(1) as row_count
from dev.retail_voucher_fact_order_detail fodbb  
group by to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date"""
logistics_L1_query="""select to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date as month, 'Logistics' as domain_name, 'OD_with_L1_query' as dashboard, 'logistics_fact_order_detail' as view_name,
COUNT(distinct concat("order_id", '_',"transaction_id",'_',"bap_id",'_',"bpp_id",'_',CAST("order_date" AS varchar))) as distinct_order_count,
count(1) as row_count
from dev.logistics_fact_order_detail
group by to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date"""

B2C_L2_query="""select to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date as month, 'B2C' as domain_name, 'OD_with_L2_query' as dashboard, 'district_level_orders' as view_name,
sum(total_orders_delivered) as distinct_order_count,
count(1) as row_count
from dev.district_level_orders dlo  
group by to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date"""
B2B_L2_query="""select to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date as month, 'B2B' as domain_name, 'OD_with_L2_query' as dashboard, 'retail_b2b_district_level_orders' as view_name,
sum(total_orders_delivered) as distinct_order_count,
count(1) as row_count
from dev.retail_b2b_district_level_orders  
group by to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date"""
GV_L2_query="""select to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date as month, 'Voucher' as domain_name, 'OD_with_L2_query' as dashboard, 'retail_voucher_district_level_orders' as view_name,
sum(total_orders_delivered) as distinct_order_count,
count(1) as row_count
from dev.retail_voucher_district_level_orders rvdlo 
group by to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date"""
logistics_L2_query="""select to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date as month, 'Logistics' as domain_name, 'OD_with_L2_query' as dashboard, 'logistics_district_level_orders' as view_name,
sum(total_orders_delivered) as distinct_order_count,
count(1) as row_count
from dev.logistics_district_level_orders ldlo 
group by to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date"""

L3_query_Retail="""select concat(order_year ,'-',order_month,'-','01') as month, sub_domain  as domain_name, 'OD_with_L3_query' as dashboard, 'district_wise_monthly_aggregates' as view_name,
sum(total_orders_delivered) as distinct_order_count,
count(1) as row_count
from dev.district_wise_monthly_aggregates dwma where domain_name = 'Retail'
group by sub_domain, concat(order_year ,'-',order_month,'-','01')"""
L3_query_Logistics="""select concat(order_year ,'-',order_month,'-','01') as month, domain_name  as domain_name, 'OD_with_L3_query' as dashboard, 'district_wise_monthly_aggregates' as view_name,
sum(total_orders_delivered) as distinct_order_count,
count(1) as row_count
from dev.district_wise_monthly_aggregates dwma where domain_name = 'Logistics'
group by domain_name, concat(order_year ,'-',order_month,'-','01')"""

OD_NO_validation_query="""select date_format(date_parse("o_created date & time" , '%Y-%m-%dT%H:%i:%s'), '%Y-%m') as month, 'B2C' as domain_name, 'NO_without_base_query' as dashboard, 'shared_order_fulfillment_nhm_fields_view_hudi' as view_name,
count(1) as row_count,
...
union
select date_format(date_parse("o_created date & time" , '%Y-%m-%dT%H:%i:%s'), '%Y-%m') as month, 'B2B' as domain_name, 'NO_without_base_query' as dashboard,'shared_b2b_order_fullfillment_view' as view_name,
count(1) as row_count,
count(distinct "network order id") as distinct_order_count
from "default".shared_b2b_order_fullfillment_view
group by date_format(date_parse("o_created date & time" , '%Y-%m-%dT%H:%i:%s'), '%Y-%m')
union
select date_format(date_parse(CAST(o_created_at_date AS VARCHAR), '%Y-%m-%d %H:%i:%s.%f'), '%Y-%m') as month, 'Voucher' as domain_name, 'NO_without_base_query' as dashboard,'shared_gift_card_order_fulfillment' as view_name,
count(1) as row_count,
count(distinct "network_order_id") as distinct_order_count
...
union
select date_format(date_parse(CAST("order_created_at" as VARCHAR), '%Y-%m-%dT%H:%i:%s'), '%Y-%m') as month, 'Logistics' as domain_name, 'NO_without_base_query' as dashboard,'shared_logistics_item_fulfillment_view_with_date' as view_name,
count(1) as row_count,
...
from "default".shared_logistics_item_fulfillment_view_with_date
group by date_format(date_parse(CAST("order_created_at" as VARCHAR), '%Y-%m-%dT%H:%i:%s'), '%Y-%m')
union
select date_format(date_parse("o_created date & time" , '%Y-%m-%dT%H:%i:%s'), '%Y-%m') as month, 'B2C' as domain_name, 'OD_without_base_query' as dashboard, 'nhm_order_fulfillment_subset_v1' as view_name,
count(1) as row_count,
count(distinct "network order id") as distinct_order_count
from "default".nhm_order_fulfillment_subset_v1
group by date_format(date_parse("o_created date & time" , '%Y-%m-%dT%H:%i:%s'), '%Y-%m')
union
select date_format(date_parse("o_created date & time" , '%Y-%m-%dT%H:%i:%s'), '%Y-%m') as month, 'B2B' as domain_name, 'OD_without_base_query' as dashboard,'shared_open_data_b2b_order' as view_name,
count(1) as row_count,
count(distinct "network order id") as distinct_order_count
from "default".shared_open_data_b2b_order
group by date_format(date_parse("o_created date & time" , '%Y-%m-%dT%H:%i:%s'), '%Y-%m')
union
select date_format(date_parse(CAST(o_created_at_date AS VARCHAR), '%Y-%m-%d %H:%i:%s.%f'), '%Y-%m') as month, 'Voucher' as domain_name, 'OD_without_base_query' as dashboard,'shared_open_data_gift_voucher_order' as view_name,
count(1) as row_count,
count(distinct "network order id") as distinct_order_count
from "default".shared_open_data_gift_voucher_order
group by date_format(date_parse(CAST(o_created_at_date AS VARCHAR), '%Y-%m-%d %H:%i:%s.%f'), '%Y-%m')
union
select date_format(date_parse(CAST("order_created_at" as VARCHAR), '%Y-%m-%d %H:%i:%s.%f'), '%Y-%m') as month, 'Logistics' as domain_name, 'OD_without_base_query' as dashboard,'shared_open_data_logistics_order' as view_name,
count(1) as row_count,
COUNT(distinct concat("order_id", '_',"transaction_id",'_',"bap_id",'_',"bpp_id",'_',CAST("order_created_at" AS varchar))) as distinct_order_count
from "default".shared_open_data_logistics_order
group by date_format(date_parse(CAST("order_created_at" as VARCHAR), '%Y-%m-%d %H:%i:%s.%f'), '%Y-%m')"""



GV_OD_query_LRD="""select date_format(date_parse(cast("order_date" as varchar) , '%Y-%m-%d'), '%Y-%m') as month, 'Voucher' as domain_name, 'OD_with_base_query' as dashboard, 'shared_open_data_gift_voucher_order' as view_name,
...
WHERE odv."seller np name" NOT IN ('gl-6912-httpapi.glstaging.in/gl/ondc')
and (DATE(odv."o_created_at_date")) >= DATE('2023-11-01')
GROUP BY odv."network order id") where cast("order_date" as varchar) < '{run_date}'
group by date_format(date_parse(cast("order_date" as varchar) , '%Y-%m-%d'), '%Y-%m') """

B2B_OD_query_LRD="""select date_format(date_parse(cast("order_date" as varchar) , '%Y-%m-%d'), '%Y-%m') as month, 'B2B' as domain_name, 'OD_with_base_query' as dashboard, 'shared_open_data_b2b_order' as view_name,
count(1) as row_count_till_last_run_date,
count(distinct "network_order_id") as distinct_order_count_till_last_run_date from(
SELECT odv."network order id" AS network_order_id,
    MAX(odv."seller np name") AS seller_np,
...
B2C_OD_query_LRD="""select date_format(date_parse(cast("order_date" as varchar) , '%Y-%m-%d'), '%Y-%m') as month, 'B2C' as domain_name, 'OD_with_base_query' as dashboard, 'nhm_order_fulfillment_subset_v1' as view_name,
count(1) as row_count_till_last_run_date,
count(distinct "network_order_id") as distinct_order_count_till_last_run_date from(
WITH order_subcategories AS (
    select
...
WHERE odv."seller np name" NOT IN ('gl-6912-httpapi.glstaging.in/gl/ondc')
and date(date_parse("O_Created Date & Time",'%Y-%m-%dT%H:%i:%s')) >= DATE('2023-11-01')
GROUP BY odv."network order id", odv."Item Category") where cast("order_date" as varchar) < '{run_date}'
group by date_format(date_parse(cast("order_date" as varchar) , '%Y-%m-%d'), '%Y-%m')"""

logistics_OD_query_LRD="""select date_format(date_parse(cast("order_date" as varchar) , '%Y-%m-%d'), '%Y-%m') as month, 'Logistics' as domain_name, 'OD_with_base_query' as dashboard, 'shared_open_data_logistics_order' as view_name,
count(1) as row_count_till_last_run_date,
...
where cast("order_date" as varchar) < '{run_date}'
group by date_format(date_parse(cast("order_date" as varchar) , '%Y-%m-%d'), '%Y-%m')"""

GV_NO_query_LRD="""SELECT date_format(date_parse(CAST(o_created_at_date AS VARCHAR), '%Y-%m-%d %H:%i:%s.%f'), '%Y-%m') as month,'Voucher' as domain_name, 'NO_with_base_query' as dashboard, 'shared_gift_card_order_fulfillment' as view_name,
    count(1) as row_count_till_last_run_date,
    COUNT(DISTINCT network_order_id) as distinct_order_count_till_last_run_date
FROM 
    "default".shared_gift_card_order_fulfillment
where CAST(o_created_at_date AS VARCHAR) < '{run_date}'
GROUP BY 
    date_format(date_parse(CAST(o_created_at_date AS VARCHAR), '%Y-%m-%d %H:%i:%s.%f'), '%Y-%m')"""
B2B_NO_query_LRD="""select date_format(date_parse(cast("Date" as varchar) , '%Y-%m-%d'), '%Y-%m') as month, 'B2B' as domain_name, 'NO_with_base_query' as dashboard, 'shared_order_fulfillment_nhm_fields_view_hudi' as view_name,
count(1) as row_count_till_last_run_date,
...
group by
	...
	6,
	7,
	8) where cast("Date" as varchar) < '{run_date}'
    group by date_format(date_parse(cast("Date" as varchar) , '%Y-%m-%d'), '%Y-%m')"""
B2C_NO_query_LRD="""select date_format(date_parse(cast("Date" as varchar) , '%Y-%m-%d'), '%Y-%m') as month, 'B2C' as domain_name, 'NO_with_base_query' as dashboard, 'shared_order_fulfillment_nhm_fields_view_hudi' as view_name,
count(1) as row_count_till_last_run_date,
count(distinct "Network order id") as distinct_order_count_till_last_run_date from (
with bottom_layer as (
select *,
...
select
	...
	"on_confirm_error_code"
from
	table_l) where cast("Date" as varchar) < '{run_date}'
    group by date_format(date_parse(cast("Date" as varchar) , '%Y-%m-%d'), '%Y-%m')"""

logistics_NO_query_LRD="""select date_format(date_parse(cast("Date" as varchar) , '%Y-%m-%d'), '%Y-%m') as month, 'Logistics' as domain_name, 'NO_with_base_query' as dashboard, 'shared_logistics_item_fulfillment_view_with_date' as view_name,
count(1) as row_count_till_last_run_date,
COUNT(distinct concat("order_id", '_',"transaction_id",'_',"bap_id",'_',"bpp_id",'_',CAST("order_created_at" AS varchar))) as distinct_order_count_till_last_run_date from (
with table1 as (
select
		"network order id" as "Network order id",
		row_number() over (partition by ("network order id" ||	
...
AND (CASE 
  ...
  AND (a.on_confirm_sync_response <> 'NACK' OR a.on_confirm_sync_response IS NULL)
  AND (a.on_confirm_error_code IS NULL OR a.on_confirm_error_code NOT IN ('65001', '66001'))
  AND cast("Date" as varchar) < '{run_date}'
GROUP BY date_format(date_parse(CAST("Date" as VARCHAR), '%Y-%m-%d'), '%Y-%m')
ORDER BY month"""

B2C_L1_query_LRD="""select to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date as month, 'B2C' as domain_name, 'OD_with_L1_query' as dashboard, 'fact_order_detail' as view_name,
COUNT(distinct "network_order_id") as distinct_order_count_till_last_run_date,
count(1) as row_count_till_last_run_date
from dev.fact_order_detail fod where cast("order_date" as varchar) < '{run_date}'
group by to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date"""
B2B_L1_query_LRD="""select to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date as month, 'B2B' as domain_name, 'OD_with_L1_query' as dashboard, 'fact_order_detail_b2b' as view_name,
COUNT(distinct "network_order_id") as distinct_order_count_till_last_run_date,
count(1) as row_count_till_last_run_date
from dev.fact_order_detail_b2b fodbb  where cast("order_date" as varchar) < '{run_date}'
group by to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date"""
GV_L1_query_LRD="""select to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date as month, 'Voucher' as domain_name, 'OD_with_L1_query' as dashboard, 'retail_voucher_fact_order_detail' as view_name,
COUNT(distinct "network_order_id") as distinct_order_count_till_last_run_date,
count(1) as row_count_till_last_run_date
from dev.retail_voucher_fact_order_detail fodbb  where cast("order_date" as varchar) < '{run_date}'
group by to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date"""
logistics_L1_query_LRD="""select to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date as month, 'Logistics' as domain_name, 'OD_with_L1_query' as dashboard, 'logistics_fact_order_detail' as view_name,
COUNT(distinct concat("order_id", '_',"transaction_id",'_',"bap_id",'_',"bpp_id",'_',CAST("order_date" AS varchar))) as distinct_order_count_till_last_run_date,
count(1) as row_count_till_last_run_date
from dev.logistics_fact_order_detail where cast("order_date" as varchar) < '{run_date}'
group by to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date"""

B2C_L2_query_LRD="""select to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date as month, 'B2C' as domain_name, 'OD_with_L2_query' as dashboard, 'district_level_orders' as view_name,
sum(total_orders_delivered) as distinct_order_count_till_last_run_date,
count(1) as row_count_till_last_run_date
from dev.district_level_orders dlo  where cast("order_date" as varchar) < '{run_date}'
group by to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date"""
B2B_L2_query_LRD="""select to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date as month, 'B2B' as domain_name, 'OD_with_L2_query' as dashboard, 'retail_b2b_district_level_orders' as view_name,
sum(total_orders_delivered) as distinct_order_count_till_last_run_date,
count(1) as row_count_till_last_run_date
from dev.retail_b2b_district_level_orders  where cast("order_date" as varchar) < '{run_date}'
group by to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date"""
GV_L2_query_LRD="""select to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date as month, 'Voucher' as domain_name, 'OD_with_L2_query' as dashboard, 'retail_voucher_district_level_orders' as view_name,
sum(total_orders_delivered) as distinct_order_count_till_last_run_date,
count(1) as row_count_till_last_run_date
from dev.retail_voucher_district_level_orders rvdlo where cast("order_date" as varchar) < '{run_date}'
group by to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date"""
logistics_L2_query_LRD="""select to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date as month, 'Logistics' as domain_name, 'OD_with_L2_query' as dashboard, 'logistics_district_level_orders' as view_name,
sum(total_orders_delivered) as distinct_order_count_till_last_run_date,
count(1) as row_count_till_last_run_date
from dev.logistics_district_level_orders ldlo where cast("order_date" as varchar) < '{run_date}'
group by to_timestamp(cast("order_date" as varchar), 'YYYY-MM')::date"""

OD_NO_validation_query_LRD="""select date_format(date_parse("o_created date & time" , '%Y-%m-%dT%H:%i:%s'), '%Y-%m') as month, 'B2C' as domain_name, 'NO_without_base_query' as dashboard, 'shared_order_fulfillment_nhm_fields_view_hudi' as view_name,
count(1) as row_count_till_last_run_date,
count(distinct "network order id") as distinct_order_count_till_last_run_date
from "default".shared_order_fulfillment_nhm_fields_view_hudi where "o_created date & time" < '{run_date}'
group by date_format(date_parse("o_created date & time" , '%Y-%m-%dT%H:%i:%s'), '%Y-%m')
union
select date_format(date_parse("o_created date & time" , '%Y-%m-%dT%H:%i:%s'), '%Y-%m') as month, 'B2B' as domain_name, 'NO_without_base_query' as dashboard,'shared_b2b_order_fullfillment_view' as view_name,
count(1) as row_count_till_last_run_date,
count(distinct "network order id") as distinct_order_count_till_last_run_date
from "default".shared_b2b_order_fullfillment_view where "o_created date & time" < '{run_date}'
group by date_format(date_parse("o_created date & time" , '%Y-%m-%dT%H:%i:%s'), '%Y-%m')
union
select date_format(date_parse(CAST(o_created_at_date AS VARCHAR), '%Y-%m-%d %H:%i:%s.%f'), '%Y-%m') as month, 'Voucher' as domain_name, 'NO_without_base_query' as dashboard,'shared_gift_card_order_fulfillment' as view_name,
count(1) as row_count_till_last_run_date,
count(distinct "network_order_id") as distinct_order_count_till_last_run_date
from "default".shared_gift_card_order_fulfillment where CAST(o_created_at_date AS VARCHAR) < '{run_date}'
group by date_format(date_parse(CAST(o_created_at_date AS VARCHAR), '%Y-%m-%d %H:%i:%s.%f'), '%Y-%m')
union
select date_format(date_parse(CAST("order_created_at" as VARCHAR), '%Y-%m-%dT%H:%i:%s'), '%Y-%m') as month, 'Logistics' as domain_name, 'NO_without_base_query' as dashboard,'shared_logistics_item_fulfillment_view_with_date' as view_name,
count(1) as row_count_till_last_run_date,
...
from "default".shared_logistics_item_fulfillment_view_with_date where CAST("order_created_at" AS VARCHAR) < '{run_date}'
group by date_format(date_parse(CAST("order_created_at" as VARCHAR), '%Y-%m-%dT%H:%i:%s'), '%Y-%m')
union
select date_format(date_parse("o_created date & time" , '%Y-%m-%dT%H:%i:%s'), '%Y-%m') as month, 'B2C' as domain_name, 'OD_without_base_query' as dashboard, 'nhm_order_fulfillment_subset_v1' as view_name,
count(1) as row_count_till_last_run_date,
count(distinct "network order id") as distinct_order_count_till_last_run_date
from "default".nhm_order_fulfillment_subset_v1 where "o_created date & time" < '{run_date}'
group by date_format(date_parse("o_created date & time" , '%Y-%m-%dT%H:%i:%s'), '%Y-%m')
union
select date_format(date_parse("o_created date & time" , '%Y-%m-%dT%H:%i:%s'), '%Y-%m') as month, 'B2B' as domain_name, 'OD_without_base_query' as dashboard,'shared_open_data_b2b_order' as view_name,
count(1) as row_count_till_last_run_date,
count(distinct "network order id") as distinct_order_count_till_last_run_date
from "default".shared_open_data_b2b_order where "o_created date & time" < '{run_date}'
group by date_format(date_parse("o_created date & time" , '%Y-%m-%dT%H:%i:%s'), '%Y-%m')
union
select date_format(date_parse(CAST(o_created_at_date AS VARCHAR), '%Y-%m-%d %H:%i:%s.%f'), '%Y-%m') as month, 'Voucher' as domain_name, 'OD_without_base_query' as dashboard,'shared_open_data_gift_voucher_order' as view_name,
count(1) as row_count_till_last_run_date,
count(distinct "network order id") as distinct_order_count_till_last_run_date
from "default".shared_open_data_gift_voucher_order where CAST(o_created_at_date AS VARCHAR) < '{run_date}'
group by date_format(date_parse(CAST(o_created_at_date AS VARCHAR), '%Y-%m-%d %H:%i:%s.%f'), '%Y-%m')
union
select date_format(date_parse(CAST("order_created_at" as VARCHAR), '%Y-%m-%d %H:%i:%s.%f'), '%Y-%m') as month, 'Logistics' as domain_name, 'OD_without_base_query' as dashboard,'shared_open_data_logistics_order' as view_name,
count(1) as row_count_till_last_run_date,
COUNT(distinct concat("order_id", '_',"transaction_id",'_',"bap_id",'_',"bpp_id",'_',CAST("order_created_at" AS varchar))) as distinct_order_count_till_last_run_date
from "default".shared_open_data_logistics_order where CAST("order_created_at" as VARCHAR) < '{run_date}'
group by date_format(date_parse(CAST("order_created_at" as VARCHAR), '%Y-%m-%d %H:%i:%s.%f'), '%Y-%m')"""